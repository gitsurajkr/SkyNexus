...
export default function BottomPanel({ currentData }: BottomPanelProps) {
  ...
  const health = currentData ? {
    battery: Math.round(Math.max(0, Math.min(100, ((currentData.VOLTAGE - 6.0) / (8.4 - 6.0)) * 100))),
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
        <div className="rounded-lg border-2 p-4 bg-white border-gray-300 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <tbody>
                <SensorItem label="Accel Roll" value={sensors.accelR} unit="m/s²" />
                <SensorItem label="Accel Pitch" value={sensors.accelP} unit="m/s²" />
                <SensorItem label="Accel Yaw" value={sensors.accelY} unit="m/s²" />
        ...
        <div className="rounded-lg border-2 p-4 bg-white border-gray-300 shadow-sm">
          ...
          <div className="max-h-48 overflow-y-auto">
            {currentData?.STATE === 'LANDED' && <EventItem time={currentData.MISSION_TIME} level="info" message="CanSat has landed" />}
            {currentData?.STATE === 'EGG_RELEASE' && <EventItem time={currentData.MISSION_TIME} level="info" message="Egg release sequence initiated" />}
            {currentData?.STATE === 'PAYLOAD_RELEASE' && <EventItem time={currentData.MISSION_TIME} level="warning" message="Payload released" />}
            {currentData?.STATE === 'DESCENT' && <EventItem time={currentData.MISSION_TIME} level="info" message="Descent phase active" />}
            {currentData?.STATE === 'APOGEE' && <EventItem time={currentData.MISSION_TIME} level="warning" message="Apogee detected" />}
            {currentData?.STATE === 'ASCENT' && <EventItem time={currentData.MISSION_TIME} level="info" message="Ascending" />}
            <EventItem time={currentData?.MISSION_TIME || '00:00:00'} level="info" message={`State: ${currentData?.STATE || 'LAUNCH_PAD'}`} />
            <EventItem time={currentData?.MISSION_TIME || '00:00:00'} level="info" message={`Temperature: ${currentData?.TEMPERATURE.toFixed(1) || 0}°C`} />
          </div>
        </div>
      </div>
    </div>
