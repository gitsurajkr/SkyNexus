...
export default function GraphsMapSection({ allData, currentIndex }: GraphsMapSectionProps) {
  ...
  const recentData = allData.slice(startIndex, endIndex).map((d, i, arr) => {
    ...
    // Calculate battery percentage (12.6V = 100%, 11.1V = 0%)
    const batteryPercent = Math.max(0, Math.min(100, ((d.VOLTAGE - 11.1) / (12.6 - 11.1)) * 100));
    
    // Calculate total acceleration magnitude from roll/pitch/yaw
    const gForceMagnitude = Math.sqrt(d.ACCEL_R ** 2 + d.ACCEL_P ** 2 + d.ACCEL_Y ** 2);
